...
(function() {
    const ALL_WORDS = [
      ...
      { en: 'concept', ko: '개념' }, { en: 'perspective', ko: '관점, 전망' }, { en: 'consequence', 단어ko: '결과, 중요성' },
    ...
    function renderResult() {
        let badge = { level: '필수편', color: '#10B981', bg: '#D1FAE5', text: '수능 영어의 첫 단추! 흔들림 없는 기본기를 위한 필수 어휘부터 시작하세요.' };
        if(score >= 8) badge = { level: '실력편', color: '#8B5CF6', bg: '#EDE9FE', text: '상위 10% 최상위권의 정점! 만점을 향한 어휘 학습이 필요합니다.' };
        else if (score >= 6) badge = { level: '핵심편', color: '#3B82F6', bg: '#DBEAFE', text: '꾸준한 1등급의 뼈대를 세워야 할 때! 고빈출 핵심 어휘에 집중하세요.' };

        container.innerHTML = \`
            <div style="font-size: 6rem; margin-bottom: 10px; filter: drop-shadow(0 10px 10px rgba(0,0,0,0.1));">🎯</div>
            <h2 style="font-size: 2.2rem; font-weight: 900; color: #0F172A; margin-bottom: 10px; letter-spacing: -1px;">\${timeLeft <= 0 ? 'TIME OUT!' : '테스트 완료!'}</h2>
            <div style="font-size: 5rem; font-weight: 900; color: \${badge.color}; margin-bottom: 30px; letter-spacing: -2px; text-shadow: 0 10px 20px \${badge.color}30;">
                \${score * 10}<span style="font-size: 2rem;">점</span>
            </div>
            
            <div style="background: \${badge.bg}; border: 2px solid \${badge.color}30; border-radius: 24px; padding: 35px 30px; margin-bottom: 50px;">
                <span style="display: inline-block; background: \${badge.color}; color: white; padding: 10px 24px; border-radius: 30px; font-weight: 800; font-size: 1.1rem; margin-bottom: 15px; box-shadow: 0 10px 20px -5px \${badge.color}60;">
                    수단비 \${badge.level} 추천
                </span>
                <p style="font-size: 1.25rem; color: #1E293B; font-weight: 600; line-height: 1.6; margin: 0; word-break: keep-all;">
                    \${badge.text}
                </p>
            </div>
            
            <div style="border-top: 1px dashed #CBD5E1; padding-top: 40px;">
                <h3 style="font-size: 1.6rem; font-weight: 900; color: #0F172A; margin-bottom: 10px;">🏆 주간 전국 랭킹 등록하기</h3>
                <p style="color: #64748B; font-size: 1.1rem; margin-bottom: 25px;">이메일을 입력하고 다른 수험생들과 순위를 비교해보세요!</p>
                
                <form onsubmit="sdHandleEmailSubmit(event)" style="display: flex; gap: 15px; justify-content: center; max-width: 550px; margin: 0 auto;">
                    <input id="sd-email-input" type="email" required placeholder="수험생님의 이메일 입력" style="flex: 1; padding: 18px 24px; border-radius: 16px; border: 2px solid #E2E8F0; font-size: 1.2rem; font-weight: 500; outline: none; transition: all 0.3s;" onfocus="this.style.borderColor='#3B82F6'; this.style.boxShadow='0 0 0 4px rgba(59,130,246,0.1)'" onblur="this.style.borderColor='#E2E8F0'; this.style.boxShadow='none'">
                    <button id="sd-email-submit" type="submit" style="background: #0F172A; color: white; font-weight: 800; padding: 0 35px; border-radius: 16px; border: none; cursor: pointer; font-size: 1.2rem; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">등록 및 순위 확인</button>
                </form>
            </div>
        \`;
    }
...
paths.forEach(p => {
    ...
    // 1. Delete old section starting from <!-- Level Test Minigame Section --> to </script> before footer
    const startIndex = txt.indexOf('<!-- Level Test Minigame Section');
    if(startIndex !== -1) {
        let endIndex = txt.indexOf('</script>', startIndex);
        if(endIndex !== -1) {
    ...
    // 2. Insert new enhanced minigame right BEFORE <!-- NEW: Sudanbi Vertical Stack Roadmap -->
    const targetStr = '<!-- NEW: Sudanbi Vertical Stack Roadmap -->';
    const insertIndex = txt.indexOf(targetStr);
    
    if(insertIndex !== -1) {
        // Insert right above it
        txt = txt.substring(0, insertIndex) + NEW_MINIGAME_HTML + '\n\n      ' + txt.substring(insertIndex);
    }
    
    fs.writeFileSync(p, txt, 'utf8');
    console.log('Successfully enhanced and moved minigame in', p);
});
